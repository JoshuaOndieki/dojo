...
class TestCreateRoom(unittest.TestCase):
    ...
    def test_create_rooms_successfully(self):
        ...
        self.assertEqual(success('An office called Spire has been created successfully!'),spire_office)
        self.assertEqual(success('A Livingspace called Amity has been created successfully!'),amity_livingspace)
    ...
    def test_it_fails_with_existing_room(self):
        ...
        self.assertTrue(try_overwrite_o == error('Room Amhere exists!') and try_overwrite_l == error('Room Amhere exists!'))

    def test_it_fails_with_invalid_room_type(self):
        invalid_type = self.dojo.create_room('Amhere','Kitchen')
        self.assertEqual(error('Only offices and livingspaces allowed!'),invalid_type)
    ...
    def test_fails_with_room_name_not_string(self):
        ...
        self.assertEqual(invalid_room,error('Room names can only be strings!'))
        self.assertEqual(invalid_room1,error('Room names can only be strings!'))
        self.assertEqual(invalid_room2,error('Room names can only be strings!'))
        self.assertEqual(invalid_room3,error('Room names can only be strings!'))
